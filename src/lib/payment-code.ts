/**
 * Payment-code domain rules — pure, framework-free, shared by the App Router
 * (server actions) and the storefront checkout UI.
 *
 ...
 * source; production server code passes a CSPRNG (`crypto.randomInt`).
 */
...
export function generatePaymentCode(
  random: RandomIndexSource = () => Math.floor(Math.random() * PAYMENT_CODE_ALPHABET.length),
): string {
  const groups: string[] = [];
...
/**
 * Calls `insert` with freshly generated values until one succeeds without a
